(function () {
	...
	/**
	 * Introduce the catalogApp.product.list.detail submodule
	 * and configure it.
	 *
   * @requires ui.router
	...
	resolveProductFromArray.$inject = ['Product', '$stateParams', '$log', 'products'];
	 ...
	 */
	function resolveProductFromArray(Product, $stateParams, $log, products) {
        return Product.getProductDetails({id:'productDetails', controller:$stateParams.id}, function(response){
            $log.debug('configureProductListDetail::resolveProductFromArray::respone ='+angular.toJson(response,true));
                            return response;
                        }).$promise;
    }
...
//	           if($stateParams.id===products[i].productId){
...
//	        }
    //return foundproduct;
	        //$log.debug('configureProductListDetail::resolveProductFromArray::foundproduct='+angular.toJson(foundproduct,true));
